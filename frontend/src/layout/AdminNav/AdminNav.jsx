...
const SEEN_KEY = 'admin_seen_notification_ids';
...
const AdminNav = () => {
  ...
  const buildNotifications = useCallback(async () => {
    ...
    try {
      ...
      // ── 1. New User Registrations (last 48h) ──────────────────────────
      try {
        ...
        users.forEach((u) => {
          if (u.created_at && isWithinLastTwoDays(u.created_at)) {
            candidates.push({
              id: `user-new-${u.id}`,
              type: 'new_user',
              icon: FaUserPlus,
              ...
              detail: `${u.full_name || u.username} · ${u.role || 'customer'}`,
              time: timeAgo(u.created_at),
              eventDate: u.created_at,
              path: '/admin/users',
            });
          }
        });
      ...
      try {
        ...
        orders.forEach((o) => {
          if (o.created_at && isWithinLastTwoDays(o.created_at)) {
            candidates.push({
              ...
              detail: `${o.customer_name || 'Customer'} · ${o.items?.length || 0} items`,
        ...
        // Silently fail if orders API doesn't exist yet
      ...
      // ── 4. Menu Items Out of Stock (last 48h) ─────────────────────────
      try {
        ...
        unavailable.forEach((item) => {
          if (item.updated_at && isWithinLastTwoDays(item.updated_at)) {
            candidates.push({
              ...
              detail: 'Marked unavailable by chef',
      ...
      // ── 5. Cancelled Reservations (last 48h) ──────────────────────────
      try {
        const resResponse = await getReservations();
        const reservations = Array.isArray(resResponse)
          ? resResponse
        ...
        reservations.forEach((r) => {
          if (
            ...
            candidates.push({
              ...
              color: 'text-red-500',
              bg: 'bg-red-500/10',
              title: `Reservation #${r.id} cancelled`,
        ...
        console.error('Cancelled notifications error:', err);
      }
  ...
  // ── mark all as seen when admin opens the dropdown ──────────────────
  ...
  const getUserName = () => user?.full_name || user?.username || 'Admin';
  const getUserRole = () => user?.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : 'Admin';
        ...
        {(sidebarOpen || !isMobile) && (
          <motion.aside
            ...
            className={`
              ...
              transition-all duration-300 border-r border-gray-700/50
            `}
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="h-16 flex items-center justify-between px-4 border-b border-gray-700/50">
                <div className="flex items-center gap-2 overflow-hidden">
                  <div className="w-8 h-8 bg-gradient-to-br from-primary to-primary/70 rounded-lg flex items-center justify-center flex-shrink-0 shadow-lg">
                  ...
                  {sidebarOpen && (
                    <motion.span
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="font-bold text-lg bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent whitespace-nowrap"
                    >
                      Admin Panel
                    </motion.span>
                  )}
                ...
                {isMobile && sidebarOpen && (
                  <button
                    onClick={() => setSidebarOpen(false)}
                    className="p-2 hover:bg-gray-700/50 rounded-lg transition-all md:hidden text-white"
                  >
                    <FaTimes size={18} />
                  </button>
                )}
                {!isMobile && (
                  <button
                    ...
                    className="p-2 hover:bg-gray-700/50 rounded-lg transition-all hidden md:block text-white/70 hover:text-white"
                  >
                ...
                {navigation.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon 
                        size={20} 
                        className={`flex-shrink-0 ${active ? 'text-primary' : 'text-gray-400 group-hover:text-white'}`} 
                      />
                      {sidebarOpen && (
                        <motion.span
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className={`text-sm font-medium whitespace-nowrap ${active ? 'text-white' : 'text-gray-300'}`}
                        >
                          {item.name}
                        </motion.span>
                      )}
        ...
        <header className="bg-card shadow-sm h-16 flex items-center justify-between px-4 md:px-6 border-b border-border">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-lg transition-all md:hidden text-text"
            >
              <FaBars size={20} />
            </button>
            <h1 className="text-lg md:text-xl font-semibold text-text truncate">
              {navigation.find((nav) => nav.path === location.pathname)?.name || 'Admin Dashboard'}
            </h1>
          </div>
